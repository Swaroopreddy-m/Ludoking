...
function connectSocket() {
  ...
  ws = new WebSocket(`${protocol}//${window.location.host}/ws/ludo`);
  
  ws.onopen = () => {
    console.log("WebSocket connected!");
    // Auto-rejoin on reconnect if active room is cached
    if (App.user && App.currentRoomId) {
      sendWS("joinRoom", { username: App.user.username, roomId: App.currentRoomId });
    }
  };
...
function sendWS(type, payload = {}) {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    console.error("WS not open. Failed to send:", type);
  }
}
...
const App = {
  ...
  cacheDOM() {
    ...
    // Invite fields
    this.inviteLinkVal = document.getElementById('invite-link-val');
  ...
  loginUser(user) {
    ...
    // Check if user was previously in a room or has a pending invite code
    if (this.pendingJoinCode) {
      const code = this.pendingJoinCode;
      this.pendingJoinCode = null;
      window.history.replaceState({}, document.title, window.location.pathname);
      this.showToast(`Joining room code ${code}...`, 'success');
      setTimeout(() => {
        socket.emit('joinRoom', { username: user.username, roomId: code });
      }, 500);
    } else {
...
socket.on('roomState', (room) => {
  ...
  if (room.state === 'LOBBY') {
    ...
    const inviteLink = `${window.location.protocol}//${window.location.host}/?join=${room.roomId}`;
    App.inviteLinkVal.value = inviteLink;
